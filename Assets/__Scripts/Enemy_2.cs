...
public class Enemy_2 : Enemy
{
    ...
    void Start() {
        ...
        if (Random.value > 0.5f) {
            // Изменение знака .x каждой точки переносит ее на другой кран экрана
            p0.x *= -1;
            p1.x *= -1;
        }

        // Записать в birthTime текущие время
        birthTime = Time.time;
    }
    ...
    public override void Move() {
        ...
        // u>1, значит, корабль существует дольше, чем lifeTime
        if (u > 1) {
            // Этот экземляр Enemy_2 завершил свой жизненый цикл
        ...
        // Интерполировать местолоположение между двумя точками 
        pos = (1-u)*p0+u*p1;
    }
}
