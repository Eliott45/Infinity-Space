...
public class Main : MonoBehaviour
{
    ...
    public void ShipDestroyed(Enemy e) {
        ...
        if (Random.value <= e.powerUpDropChance) {
            ...
            // Установить соответсутвующий тип WeaponType
            pu.SetType(puType);
    ...
    public void SpawnEnemy() {
        ...
        // Разместить вражеский корабль над экраном в случайно позиции x
    ...
    public void Restart() {
        // Перезагрузить Game, чтобы перезапустить игру
    ...
    /// </summary>
    /// <returns> Экземпляр WeaponDefinition или, если нет такого определения для указанного WeaponType, возвращает новый экземпляр
    ...
    static public WeaponDefinition GetWeaponDefinition(WeaponType wt) {
        ...
        // Попытка извелечь значение по отсуствующему ключу вызовет ошибку, поэтому слелующая инструкция играет важную роль.
        if(WEAP_DICT.ContainsKey(wt)) {
            return(WEAP_DICT[wt]);
        }
        // Следующая инструкция возвращает новый экземпляр WeaponDefinition с типом оружия WeaponType.none, что озночает неудачную 
        // попытку найти требуемое определение WeaponDefinition
