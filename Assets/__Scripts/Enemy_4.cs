...
public class Enemy_4 : Enemy
{
    ...
    private float timeStart; // Время созданиия этого коробля
    private float duration = 4; // Продолжительность перемещения

    private void Start() {
        // Начальная позиция уже выбрана в Main.SpawnEnemy() поэтому запишем ее как начальное значения в p0 и p1
        p0 = p1 = pos;
        InitMovement();
    ...
    {
        // Этот метод gпереопредляет Enemy.Move() и реализует линейную интерполяцию
    ...
    // Эти две функции выполняют поиск части в массиве parts по имени или по ссылке на игровой объект
    ...
    // Окрашивает в красный тольно одну часть, а не весь корабль
    ...
    void OnCollisionEnter(Collision coll) {
        ...
        switch (other.tag) {
            case "ProjectileHero":
                ...
                // Проверить, был ли корабль полность разрушен
